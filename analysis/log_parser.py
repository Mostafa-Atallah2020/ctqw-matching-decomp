...
class CTQWLogParser:
    ...
    def __init__(self):
        ...
        self.key_metrics = [
            ...
            "timing_min_per_graph",
            "timing_max_per_graph",
            "timing_total_analysis",
            ...
            "win_pauli_depth",
            "win_analysis_time",
            "win_graph_edge_count",
            "win_graph_edge_density",
            "win_graph_diameter",
            "win_graph_clique_number",
            "win_graph_max_degree",
            "win_graph_avg_degree",
            "win_graph_avg_clustering",
            "win_graph_estimated_group_size",
            "win_graph_estimated_orbit_count",
            # Win graph structure statistics
            "win_graph_bipartite_count",
            "win_graph_nonbipartite_count",
            ...
            "lose_pauli_depth",
            "lose_analysis_time",
            "lose_graph_edge_count",
            "lose_graph_edge_density",
            "lose_graph_diameter",
            "lose_graph_clique_number",
            "lose_graph_max_degree",
            "lose_graph_avg_degree",
            "lose_graph_avg_clustering",
            "lose_graph_estimated_group_size",
            "lose_graph_estimated_orbit_count",
            # Lose graph structure statistics
            "lose_graph_bipartite_count",
            "lose_graph_nonbipartite_count",
            ...
            "draw_pauli_depth",
            "draw_analysis_time",
            "draw_graph_edge_count",
            "draw_graph_edge_density",
            "draw_graph_diameter",
            "draw_graph_clique_number",
            "draw_graph_max_degree",
            "draw_graph_avg_degree",
            "draw_graph_avg_clustering",
            "draw_graph_estimated_group_size",
            "draw_graph_estimated_orbit_count",
            # Draw graph structure statistics
            "draw_graph_bipartite_count",
            "draw_graph_nonbipartite_count",
        ]
    ...
    def parse_graph_statistics_section(
        self, lines: List[str], start_idx: int, category: str
    ) -> Dict:
        ...
        while i < len(lines):
            ...
            if "Total graphs processed:" in line:
                ...
                if bipartite_match:
                    stats[f"{category.lower()}_graph_bipartite_count"] = int(
                        bipartite_match.group(1)
                    )
                if nonbipartite_match:
                    stats[f"{category.lower()}_graph_nonbipartite_count"] = int(
                        nonbipartite_match.group(1)
                    )
            ...
            # Parse mean values from existing patterns
                ...
                if "Edge count" in line:
                    ...
                    match = re.search(r"Mean:\s*([0-9.]+)", line)
                    if match:
                        stats[f"{category.lower()}_graph_estimated_group_size"] = float(
                            match.group(1)
                        )
                    ...
                    match = re.search(r"Mean:\s*([0-9.]+)", line)
                    if match:
                        stats[f"{category.lower()}_graph_estimated_orbit_count"] = float(
                            match.group(1)
                        )
    ...
    def parse_log_file(
        self, log_file: str, start_run: int = 1, end_run: Optional[int] = None
        ...
        for i in range(start_run - 1, end_run):
            ...
            if run_data:
                ...
                print(
                    f"✓ Parsed run {i + 1}: {run_data.get('total_processed', 'N/A')} graphs processed"
                )
            else:
        ...
        for metric in self.key_metrics:
            ...
            for run in runs_data:
                if metric in run and run[metric] is not None:
                    ...
                    if (
                        value != 0.0
                        or metric.endswith("_percentage")
                        or metric.endswith("_count")
                        or metric == "total_processed"
                        or metric.startswith("timing_")
                    ):
                        values.append(value)
    ...
    def save_results(
        ...
        with open(summary_file, "w", newline="") as f:
            ...
            for metric, stat_data in stats.items():
                if metric not in exclude_from_summary:
                    ...
                    writer.writerow([metric, mean_str, std_str, count_str, values_str])
        ...
        if runs_data:
            ...
            other_columns = sorted(
                [k for k in all_keys if k != "run_num" and not k.startswith("config_")]
            )
    ...
    def print_summary(self, stats: Dict, config_data: Dict, runs_count: int):
        ...
        for category in categories:
            ...
            if matching_metric in stats and pauli_metric in stats:
                ...
                print(
                    f"    Matching: {matching_stat['mean']:.2f} ± {matching_stat['std']:.2f} (n={matching_stat['count']})"
                )
                print(
                    f"    Pauli:    {pauli_stat['mean']:.2f} ± {pauli_stat['std']:.2f} (n={pauli_stat['count']})"
                )
        ...
        for category in categories:
            ...
            if bipartite_metric in stats and nonbipartite_metric in stats:
                ...
                print(
                    f"    Bipartite:     {bipartite_stat['mean']:.1f} ± {bipartite_stat['std']:.1f} (n={bipartite_stat['count']})"
                )
                print(
                    f"    Non-bipartite: {nonbipartite_stat['mean']:.1f} ± {nonbipartite_stat['std']:.1f} (n={nonbipartite_stat['count']})"
                )
...
def main():
    ...
    parser.add_argument(
        "--start-run", "-s", type=int, default=1, help="Starting run number (1-indexed)"
    )
    parser.add_argument(
        "--end-run",
        "-e",
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        default="./analysis/outputs/matching_vs_pauli/stats",
        help="Output directory for results",
    )
    parser.add_argument(
        "--prefix", "-p", type=str, default="ctqw_stats", help="Prefix for output files"
    )
    ...
    # Parse log file
    runs_data, config_data = parser_obj.parse_log_file(args.log_file, args.start_run, args.end_run)
    ...
    parser_obj.save_results(
        stats, config_data, runs_data, args.output_dir, args.log_file, args.prefix
    )
