...
def trotterized_quantum_walk(graph, time, n_steps, matching_algorithm="first-fit-greedy"):
    ...
    if matching_algorithm not in ["first-fit-greedy", "hamming-distance-greedy"]:
        raise ValueError(
            f"Invalid matching algorithm: {matching_algorithm}. "
            f"Valid options are: 'first-fit-greedy', 'hamming-distance-greedy'"
        )

    # Map new names to internal names
    algorithm_mapping = {"first-fit-greedy": "greedy", "hamming-distance-greedy": "parallel"}
...
def analyze_graphs(graphs, times, n_steps_list, selected_methods, verbose=True):
    ...
    for method in selected_methods:
        results[method] = {t: {N: [] for N in n_steps_list} for t in times}
...
def create_comprehensive_plot(
    all_results, vertex_sizes, times, n_steps_list, selected_methods, save_dir, verbose=True
):
    ...
    for n_vertices in vertex_sizes:
        ...
        for method_idx, method_key in enumerate(selected_methods):
            ...
            for t_idx, t in enumerate(times):
                ...
                for N in N_values:
                    if (
                        method_key in vertex_results
                        and t in vertex_results[method_key]
                        and N in vertex_results[method_key][t]
                    ):
        ...
        # Set title
        ax.set_title(
            f"Quantum Walk Analysis: {n_vertices} Vertices\nOperator Difference Norm vs Exact CTQW",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
...
def print_summary_statistics(all_results, selected_methods, verbose=True):
    ...
    for vertex_key, vertex_results in all_results.items():
        ...
        selected_method_info = [
            (method, method_display_names[method]) for method in selected_methods
        ]
                ...
                for N in [10, 50]:  # Representative step counts
                    ...
                    for method_key, method_name in selected_method_info:
                        if (
                            ...
                            and t in vertex_results[method_key]
                            and N in vertex_results[method_key][t]
                            and vertex_results[method_key][t][N]
                        ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze operator difference norms between quantum walk implementations"
    )
