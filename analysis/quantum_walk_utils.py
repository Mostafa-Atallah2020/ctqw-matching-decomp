...
class Logger:
    ...
    def log_labeling_comparison(self, graph_index: int, original_cost: int, degree_based_cost: int,
                               original_metrics: Optional[CircuitMetrics], degree_based_metrics: Optional[CircuitMetrics]):
        """Log labeling method comparison results."""
        self.log(f"Graph {graph_index} Labeling Comparison:")
        improvement = original_cost - degree_based_cost
        self.log(f"Original Hamming cost: {original_cost}")
        self.log(f"Degree-based Hamming cost: {degree_based_cost}")
        self.log(f"Hamming improvement: {improvement}")
        
        if original_metrics and degree_based_metrics:
            cx_improvement = original_metrics.cx_count - degree_based_metrics.cx_count
            u3_improvement = original_metrics.u3_count - degree_based_metrics.u3_count
            depth_improvement = original_metrics.depth - degree_based_metrics.depth
    ...
    def log_labeling_summary(self, total_graphs: int, total_hamming_improvement: int, 
        ...
        """Log summary statistics for degree-based labeling."""
        self.log("Degree-Based Labeling Summary Statistics:")
        self.log("=" * 40)
...
class DegreeBasedHypercubeLabeler:
    """
    Implementation of the degree-based hypercube labeling algorithm from the pseudocode:
    
    Let M = vertex with maximum degree, Δ(G)
    X = ceiling(log_2(n))
    Assign string z to M
        For u in N(M), if u has no label
        Assign string to u such that d_H(b(u), b(M)) = 1
        For x in N(M) ∩ N(u) such that x has no label,
            If N(u) ∩ N(x) is not empty,
                Assign string to x such that d_H(b(u), b(x)) = 1 and d_H(b(x), b(v)) = 1
        Repeat
    """
    ...
    def degree_based_hypercube_labeling(graph: nx.Graph) -> Dict[int, str]:
        """
        Implementation of the degree-based pseudocode algorithm with bit flip tracking.
        """
        ...
        # Find M = vertex with maximum degree Δ(G)
        degrees = dict(graph.degree())
        M = max(degrees.keys(), key=lambda v: degrees[v])
        
        # Initialize labeling and tracking
        labeling = {}
        used_labels = set()
        bit_positions_used = set()  # Track which bit positions have been flipped from M
        ...
        # Get neighbors of M sorted by degree (highest first for better structure)
        ...
        for u in neighbors_M:
            if u not in labeling:
                # Find a bit position to flip that hasn't been used yet
                bit_to_flip = None
                ...
                if bit_to_flip is not None:
                    # Create label by flipping the chosen bit
                    u_label = list(z)
                    ...
                    # Fallback: find any available Hamming-1 neighbor
        ...
        while queue:
            ...
            for neighbor in unprocessed_neighbors:
                if neighbor not in labeling:
                    # Try to assign a Hamming-1 label
                    available_h1 = DegreeBasedHypercubeLabeler.get_hamming_neighbors(current_label, used_labels)
                    
                    if available_h1:
                        # Choose the best label based on minimizing cost to existing neighbors
                        best_label = None
                        min_cost = float('inf')
        ...
        # Handle any remaining unlabeled vertices
        remaining_vertices = [v for v in nodes if v not in labeling]
        
        for vertex in remaining_vertices:
            # Find the best available label by minimizing total cost to neighbors
            best_label = None
            min_cost = float('inf')
            
            # Try all available labels
            ...
            if best_label:
                ...
                # Ultimate fallback: assign any available label
                for candidate_i in range(2**num_bits):
                    candidate = format(candidate_i, f'0{num_bits}b')
                    if candidate not in used_labels:
...
class HeuristicHypercubeLabeler:
    """
    Heuristic hypercube labeler that uses the degree-based approach.
    """
    ...
    def heuristic_hypercube_labeling(graph: nx.Graph) -> Dict[int, str]:
        """Find hypercube labeling using the degree-based heuristic algorithm."""
        nodes = list(graph.nodes())
        num_nodes = len(nodes)
        ...
        # Use the degree-based heuristic algorithm
        try:
            ...
            print(f"Warning: Degree-based heuristic failed: {e}")
        
        # Fallback: degree-sorted sequential labeling (maintains degree-based philosophy)
        print(f"Using degree-sorted fallback for graph with {num_nodes} nodes")
        num_bits = max(1, math.ceil(math.log2(num_nodes)))
...
class GraphProcessor:
    ...
    def graph_to_bitstring_hypercube(graph: nx.Graph) -> Set[Tuple[str, str]]:
        """Convert graph to bitstring with degree-based hypercube labeling."""
    ...
    def compare_labeling_methods(graph: nx.Graph) -> Dict[str, Dict]:
        """Compare original vs degree-based hypercube labeling methods and their Hamming costs."""
        results = {}
        ...
        # Degree-based method
        try:
            degree_based_edges = GraphProcessor.graph_to_bitstring_hypercube(graph)
            degree_based_cost = sum(HeuristicHypercubeLabeler.hamming_distance(u, v) for u, v in degree_based_edges)
            
            # Get detailed analysis of the labeling
            degree_based_labeling = HeuristicHypercubeLabeler.heuristic_hypercube_labeling(graph)
            quality_stats = DegreeBasedHypercubeLabeler.analyze_labeling_quality(graph, degree_based_labeling)
            
            results['degree_based'] = {
                'edges': degree_based_edges, 
                'cost': degree_based_cost,
                'quality_stats': quality_stats
            }
        except Exception as e:
            results['degree_based'] = {'edges': None, 'cost': float('inf'), 'error': str(e)}
...
class HypercubeLabelingAnalyzer(BaseAnalyzer):
    """Analyzer that uses degree-based hypercube embedding labeling."""
