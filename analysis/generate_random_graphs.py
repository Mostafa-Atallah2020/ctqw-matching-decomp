...
def generate_test_graphs(n_vertices, n_graphs=100, seed=None):
    ...
    if len(graphs) < n_graphs:
        print(f"Warning: Only generated {len(graphs)} graphs out of {n_graphs} requested after {attempts} attempts")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate random connected graphs for quantum walk operator difference norm analysis')
    ...
    parser.add_argument('--force', action='store_true',
                        help='Overwrite existing graph files')
    ...
    for n_vertices in args.vertex_sizes:
        ...
        # Save graphs
        saved_filepath = save_graphs(graphs, n_vertices, args.n_graphs, args.output_dir, args.verbose)
