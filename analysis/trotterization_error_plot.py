...
def merge_data(data_dirs):
        ...
        try:
            ...
            if not merged_metadata:
                ...
                if "time_values" in merged_metadata and "time_values" in metadata:
                    merged_metadata["time_values"] = list(
                        set(merged_metadata["time_values"] + metadata["time_values"])
                    )
    ...
    if duplicates.any():
        print(
            f"Warning: Found duplicate (time, trotter_steps) combinations. Using first occurrence."
        )
...
def create_multi_dataset_plot(all_data, args, output_dir):
    ...
    plt.suptitle(
        args.title if args.title else "Trotterization Error Comparison Across Datasets", fontsize=14
    )
    plt.tight_layout()
...
def main():
    ...
    parser.add_argument(
        "data_dirs", nargs="+", help="Path(s) to data directory (output from compute script)"
    )
    parser.add_argument(
        "--error-type",
